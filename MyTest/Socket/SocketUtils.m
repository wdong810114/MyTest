...
+ (NSData *)byteFromUInt8:(uint8_t)val
{
    ...
    return [SocketUtils dataWithReverse:valData];
}
...
{
    NSData *tempData = [SocketUtils bytesFromValue:value byteCount:byteCount];
    if(!reverse) {
        return tempData;
    }
    
    return [SocketUtils dataWithReverse:tempData];
}
...
+ (uint16_t)uint16FromBytes:(NSData *)fData
{
    ...
    NSData *data = [SocketUtils dataWithReverse:fData];
    uint16_t val0 = 0;
    uint16_t val1 = 0;
...
+ (NSInteger)valueFromBytes:(NSData *)data reverse:(BOOL)reverse
{
    ...
    if(reverse) {
        tempData = [SocketUtils dataWithReverse:tempData];
    }
    return [SocketUtils valueFromBytes:tempData];
}
